...
| 檔案 | 用途 |
|---|---|
| **`START_PATCHES.cmd`** | **兩臺分工用**：這臺只錄 `devices.json` 裡的貼片 |
| **`START_POLARS.cmd`** | **兩臺分工用**：這臺只錄 `devices.json` 裡的手環 |
| `COMBINE.cmd` | 把另一臺複製過來的 `data` 檔案和這臺的合成一張總表 `merged_all_<日期>.csv` |
| `CLOCK_CHECK.cmd` | 檢查電腦時鐘和網路時間差多少（兩臺分工時必做） |
| `START_ALL.cmd` | **同一個視窗同時收錄貼片與手環**，名單來自 `devices.json`（目前 3 貼片 + 2P～7P）。沒連到的裝置會一直找，接上就自動開始錄；手環電量低於 20% 會在畫面警告。結束時產生 `merged_all_*.csv` 總表 |
...
一臺筆電的藍牙同時最多約 **9 條連線**（實測 Intel 晶片），`START_ALL.cmd` 超過時會保留全部貼片、手環依編號取到滿為止。
6 貼片 + 6 手環要分兩臺電腦收，
兩臺都開啟 Windows 自動對時（設定 → 時間與語言 → 立即同步）。

### 兩臺電腦分工（6 貼片 + 6 手環）

一臺藍牙卡連不了 12 顆（Intel 內建上限 9、TP-Link USB 卡上限 8），所以貼片和手環各用一臺。

| | 筆電（Intel 內建） | 教室主機（TP-Link USB 卡） |
|---|---|---|
| 負責 | 6 顆貼片 | 6 顆手環 |
| 雙擊 | `START_PATCHES.cmd` | `START_POLARS.cmd` |

1. 兩臺都先雙擊 `CLOCK_CHECK.cmd`，偏差要在 1 秒內；不行就到「設定 → 時間與語言 → 日期和時間」按「立即同步」。
   開錄時程式也會自動再檢查一次並印在畫面上。
2. 兩臺各自雙擊自己的啟動檔，各自 Ctrl+C 結束。先開後開沒關係，名單裡沒連到的會一直找。
3. 收完把主機的 `data` 資料夾整個複製到筆電的 `data`（檔名有時間戳，不會互相覆蓋）。
4. 筆電雙擊 `COMBINE.cmd`，輸入日期（如 `20260923`），得到 `data\merged_all_20260923.csv`：每秒一列，12 顆並排。

`devices.json` 兩臺用同一份即可，`START_PATCHES` 只看貼片、`START_POLARS` 只看手環。
...
貼片也寫在同一個檔，用 `"type": "ecg"` 標示：

```json
  "2512-03": { "label": "E2512-03", "type": "ecg" },
  "2605-02": { "label": "E2605-02", "type": "ecg", "skip": true, "note": "這次不使用" }
```

`START_ALL.cmd` 就是照這個檔的名單錄：沒有 `skip` 的全部都要錄，開機晚了也會自動接上。
要換一顆貼片，把要用的那顆的 `skip` 行刪掉、不用的加上即可。
